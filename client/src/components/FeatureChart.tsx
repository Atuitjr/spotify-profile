...
class FeatureChart extends Component {
	static propTypes: { features: any; type: any } = {
	...
	parseData = () => {
		const { features }: any = this.props;
		const dataset = this.createDataset(features);
		this.createChart(dataset);
	};

	avg = (arr: any[]) => arr.reduce((a, b) => a + b, 0) / arr.length;

	createDataset(features: any) {
		const dataset: any = {};
		properties.forEach((prop) => {
			dataset[prop] = features.length
				? this.avg(features.map((feat: any) => feat && feat[prop]))
				: features[prop];
		});
		return dataset;
	}
	...
	createChart(dataset: any) {
		...
		const labels: any = Object.keys(dataset);
		const data: any = Object.values(dataset);
