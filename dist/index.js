...
const connect_history_api_fallback_1 = __importDefault(
...
if (cluster_1.default.isMaster) {
	...
	app.get("/login", (req, res) => {
		...
		res.redirect(
			`https://accounts.spotify.com/authorize?${querystring_1.default.stringify(
				{
					response_type: "code",
					client_id: CLIENT_ID,
					scope: scope,
		...
		if (state === null || state !== storedState) {
			...
			request_1.default.post(authOptions, (error, response, body) => {
				if (!error && response.statusCode === 200) {
					...
					res.redirect(
						`${FRONTEND_URI}/#${querystring_1.default.stringify({
							access_token,
							refresh_token,
						})}`
					);
